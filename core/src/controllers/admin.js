...
function startAdminServer(dataProvider) {
    ...
    // API: 刷新微信账号 Code（免扫码，通过 yyb-adapter 调用 yyb-go /wxapp/getCode）
    app.post('/api/accounts/:id/refresh-code', async (req, res) => {
        try {
            const resolvedId = resolveAccId(req.params.id) || String(req.params.id || '');
            if (!resolvedId) {
                return res.status(400).json({ ok: false, error: '缺少账号 ID' });
            }
            ...
            adminLogger.info('refresh-code request', { accountId: resolvedId });
            const result = await refreshAccountCode(resolvedId);

            if (provider.addAccountLog) {
                provider.addAccountLog('update', `刷新 Code 成功`, resolvedId, (result.account && result.account.name) || resolvedId);
            }

            // 如果账号正在运行，用新 Code 重启
            let restarted = false;
            if (provider.isAccountRunning && provider.isAccountRunning(resolvedId)) {
                provider.restartAccount(resolvedId);
                restarted = true;
            }

            res.json({ ok: true, data: { code: result.code, restarted } });
        } catch (e) {
            adminLogger.error('refresh-code error', { error: e.message });
            res.status(500).json({ ok: false, error: e.message });
        }
    });
