...
function createWorkerManager(options) {
    ...
    // 自动刷新 Code 防抖：同一账号 5 分钟内最多自动刷新一次，避免死循环
    const lastAutoRefreshAt = new Map();

    async function handleAutoRefreshAndRestart(accountId, worker) {
        const now = Date.now();
        const last = lastAutoRefreshAt.get(accountId) || 0;
        if (now - last < 5 * 60 * 1000) {
            log('系统', `账号 ${worker.name} 自动刷新 Code 过于频繁（5 分钟内已尝试），请手动刷新或重新扫码`, { accountId: String(accountId), accountName: worker.name });
            return;
        }
        lastAutoRefreshAt.set(accountId, now);

        log('系统', `账号 ${worker.name} 检测到登录失效，正在自动刷新 Code...`, { accountId: String(accountId), accountName: worker.name });
        try {
            const { refreshAccountCode } = require('../services/wx-code');
            const result = await refreshAccountCode(accountId);
            log('系统', `账号 ${worker.name} Code 刷新成功，正在重新启动...`, { accountId: String(accountId), accountName: worker.name });
            addAccountLog('auto_refresh_code', `账号 ${worker.name} Code 刷新成功，自动重启`, accountId, worker.name, { reason: 'ws_400' });

            // 从 store 取最新账号（code 已更新）后重启
            const { getAccounts } = require('../models/store');
            const data = getAccounts();
            const accounts = Array.isArray(data.accounts) ? data.accounts : [];
            const latest = accounts.find(a => String(a.id) === String(accountId));
            if (latest) {
                restartWorker(latest);
            } else {
                startWorker({ id: accountId, name: worker.name, code: result.code, platform: 'wx' });
            }
        } catch (e) {
            const reason = e && e.message ? e.message : String(e || '未知错误');
            log('系统', `账号 ${worker.name} 自动刷新 Code 失败: ${reason}`, { accountId: String(accountId), accountName: worker.name });
            addAccountLog('auto_refresh_code_failed', `账号 ${worker.name} 自动刷新 Code 失败: ${reason}`, accountId, worker.name, { reason: 'ws_400' });
        }
    }
    ...
    function handleWorkerMessage(accountId, msg) {
        ...
        if (msg.type === 'status_sync') {
            ...
            if (code === 400) {
                addAccountLog(
                    'ws_400',
                    `账号 ${worker.name} 登录失效，正在自动刷新 Code`,
                    accountId,
                    worker.name,
                );
                handleAutoRefreshAndRestart(accountId, worker);
            }
